...
### 1.3 全局最优性条件 (针对凸函数)
> **定理 7.1.5 (充要条件)**
> 设 $f(x)$ 是定义在 $\mathbb{R}^n$ 上的**凸函数**。则 $x^*$ 是全局极小点的**充要条件**是：
> $$\nabla f(x^*) = 0$$
>
> **意义**：对于凸函数，只要找到驻点（梯度为 0 的点），它就是全局最优解。这使得凸优化问题不需要验证二阶条件，极大简化了计算。
...
### 2.0 前置理论：Fritz John 条件 (广义必要条件)
在不确定是否满足约束规范 (LICQ) 时，需使用更通用的 Fritz John 条件。

> **定理 7.2.3 (Fritz John 条件)**
> 设 $x^*$ 是局部极小点，则存在**不全为零**的非负乘子 $w_0, w_i, v_j$，使得：
> $$w_0 \nabla f(x^*) - \sum_{i=1}^m w_i \nabla g_i(x^*) - \sum_{j=1}^l v_j \nabla h_j(x^*) = 0$$
> 且满足互补松弛条件 $w_i g_i(x^*) = 0$。
>
> **与 KKT 的区别**：多了一个目标函数的乘子 $w_0$。
> * 若 $w_0 > 0$，可两边同除以 $w_0$，退化为 KKT 条件。
> * 若 $w_0 = 0$，则问题只与约束有关（退化），通常对应于不满足 LICQ 的情况。
