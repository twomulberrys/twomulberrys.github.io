...
**几何直观**：如果 $\bar{x}$ 是局部极小点，则在 $\bar{x}$ 处的可行方向一定不是下降方向（即“想变小的地方去不了，去得了的地方不会变小”）。

> **定义 (切锥)**
> 集合 $S$ 在点 $\bar{x}$ 的切锥 $T$ 是由所有可行序列的极限方向组成的集合。
> $$T = \{ d \mid \exists x^{(k)} \in S, x^{(k)} \to \bar{x}, \lambda_k > 0, \text{s.t. } d = \lim \lambda_k (x^{(k)} - \bar{x}) \}$$
>
...
> **定理 7.2.2 (仅含不等式约束的情况)**
> 若只含不等式约束，局部最优解 $\bar{x}$ 满足 $F_0 \cap G_0 = \emptyset$。
---
...
KKT 只是候选点的筛选工具，要确定极值性质（极大/极小），需借助二阶条件。

定义 Lagrange 函数 $L(x, w, v) = f(x) - \sum w_i g_i(x) - \sum v_j h_j(x)$。
定义临界锥 $\bar{G} = \{ d \in \mathbb{R}^n \mid \nabla g_i(\bar{x})^T d = 0 (i \in I, w_i>0); \nabla g_i(\bar{x})^T d \ge 0 (i \in I, w_i=0); \nabla h_j(\bar{x})^T d = 0 \}$。

> **定理 7.2.11 (二阶必要条件)**
> 设 $\bar{x}$ 是局部最优解，存在 KKT 乘子。则对任意 $d \in \bar{G}$，都有：
> $$d^T \nabla_{xx}^2 L(\bar{x}, w, v) d \ge 0$$
> 即 Lagrange 函数的 Hesse 矩阵在临界方向上**半正定**。

> **定理 7.2.12 (二阶充分条件)**
> 若在 $\bar{x}$ 处满足 KKT 条件，且对任意非零向量 $d \in \bar{G}$，都有：
> $$d^T \nabla_{xx}^2 L(\bar{x}, w, v) d > 0$$
> 即 Lagrange 函数的 Hesse 矩阵在临界方向上**正定**，则 $\bar{x}$ 是**严格局部极小解**。

---

## 6. Lagrange 对偶理论

对偶理论将原问题（Primal）转化为对偶问题（Dual），常用于求下界或简化计算。

### 6.1 对偶问题定义
原问题：$\min f(x)$, s.t. $g(x) \ge 0, h(x)=0, x \in D$。
**对偶函数**：$\theta(w, v) = \inf \{ f(x) - \sum w_i g_i(x) - \sum v_j h_j(x) \mid x \in D \}$。
**对偶问题**：
$$
\max \quad \theta(w, v) \quad \text{s.t.} \quad w \ge 0
$$


### 6.2 弱对偶与强对偶
> **定理 7.3.1 (弱对偶定理)**
> 设 $x$ 是原问题可行解，$(w, v)$ 是对偶问题可行解，则：
> $$f(x) \ge \theta(w, v)$$
> 推论：原问题的最优值 $\ge$ 对偶问题的最优值。

> **定理 7.3.3 (强对偶定理)**
> 若问题是**凸规划**且满足 **Slater 条件**（存在内点使 $g(x)>0$），则原问题与对偶问题最优值相等，且对偶问题可达最优解。

### 6.3 鞍点定理
> **定义 7.3.1 (鞍点)**
> 若 $(\bar{x}, \bar{w}, \bar{v})$ 满足 $L(\bar{x}, w, v) \le L(\bar{x}, \bar{w}, \bar{v}) \le L(x, \bar{w}, \bar{v})$，则称其为 Lagrange 函数的鞍点。

> **定理 7.3.4 (鞍点定理)**
> $(\bar{x}, \bar{w}, \bar{v})$ 是鞍点的充要条件是：$\bar{x}$ 是原问题最优解，$(\bar{w}, \bar{v})$ 是对偶问题最优解，且二者最优值相等（强对偶成立）。

> **定理 7.3.5 (鞍点与 KKT)**
> 在凸规划下，$\bar{x}$ 是最优解 $\iff$ 存在乘子使其成为鞍点 $\iff$ 满足 KKT 条件。
