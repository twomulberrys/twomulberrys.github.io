...
>
> 1.  **分离不等式退化**：
>     如果 $w_0 = 0$，Fritz John 条件中的分离超平面变得与目标函数 $f(x)$ 无关，仅取决于约束条件。这意味着分离超平面是**垂直**的。对于数值集合 $\mathcal{A}$ 中的所有点（即比最优解更“好”的点），分离不等式退化为：
>     $$
>     \sum_{i=1}^m w_i g_i(x) + \sum_{j=1}^l v_j h_j(x) \ge 0
>     $$
>     *(注意：这里利用了最优解处的值归一化为 0 的性质简化推导)*
>
> 2.  **引入 Slater 点**：
>     根据 Slater 条件，存在一个严格内点 $\hat{x}$ 使得：
>     * $g_i(\hat{x}) < 0$ （严格小于 0）
>     * $h_j(\hat{x}) = 0$ （等式成立）
>
> 3.  **导出矛盾**：
...
>     $$
>     由于 $w_i \ge 0$ 且不全为零（在 $w_0=0$ 的前提下），上述求和项必然 **严格小于 0**。
>     这与分离不等式要求的 $\ge 0$ 矛盾！
>
> **结论**：假设不成立，故必然有 $w_0 > 0$。此时可将方程两边同除以 $w_0$，从而得到标准的 KKT 条件。
